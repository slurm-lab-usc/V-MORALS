...
from cart_dataloader import CartpoleSequenceDataset
...
dataset = CartpoleSequenceDataset(data_dir=DATA_DIR, stack_size=STACK_SIZE, tau=1, max_trajectories=1152)
dataloader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True)
NUM_SAMPLES = len(dataset)
latent_space_samples = []

print("Collecting latent space samples...")
with torch.no_grad():
    samples_collected = 0
    pbar = tqdm(total=NUM_SAMPLES, desc="Sampling")
    for stack1, _, _ in dataloader:
        if samples_collected >= NUM_SAMPLES:
            break
        stack1 = stack1.to(device).float()
        z0 = encoder(stack1)
        latent_space_samples.append(z0.cpu().numpy())
        samples_collected += 1
        pbar.update(1)
    pbar.close()
...
def compute_roa(map_graph, morse_graph, lower_bounds, upper_bounds, output_dir):
    ...
    fig, ax = PlotRoA.PlotRoA(lower_bounds, upper_bounds, from_file="MG", dir_path=output_dir)
