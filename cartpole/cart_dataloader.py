...
traj_success_file = "cartpole_data_success.txt"
...
class TrajectoryRollout(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if match:
            label_key = f"trajectory_{match.group(1)}.txt"
        else:
...
class CartpoleSequenceDataset(Dataset):
    def __init__(self, data_dir, stack_size=10, tau=1, transform=None, label_file=traj_success_file, max_trajectories=None):
        self.data_dir = data_dir
        self.stack_size = stack_size
        self.tau = tau
        self.transform = transform
...
def test_label_reading():
    ...
    dataset = CartpoleSequenceDataset(data_dir=data_dir, stack_size=10, tau=970, label_file=label_file)
