...
def main():
    ...
    dataset = PendulumSequenceDataset(data_dir=DATA_DIR, stack_size=STACK_SIZE, tau=TAU, label_file=label_file, max_trajectories=818)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i, (stack1_batch, stack2_batch, label_batch) in enumerate(loop):
            ...
            # if i == 0:
            #     orig_stack = X_t[0].detach().cpu().numpy()  # shape: (stack_size, H, W)
            #     recon_stack = X_t_recon[0].detach().cpu().numpy()  # shape: (stack_size, H, W)
